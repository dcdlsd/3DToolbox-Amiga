...
PROC set_default_speech()
    StrCopy(sp_temp_ok, 'Nozzle temperature reached', ALL)
    StrCopy(sp_printing, 'Print is running', ALL)
    StrCopy(sp_paused, 'Print is paused', ALL)
    StrCopy(sp_pausing, 'Print pause requested', ALL)
    StrCopy(sp_resuming, 'Print resume requested', ALL)
    StrCopy(sp_done, 'Print completed', ALL)
    StrCopy(sp_status_ready, 'Printer ready', ALL)
    StrCopy(sp_status_offline, 'Printer offline', ALL)
    StrCopy(sp_status_received, 'Printer status received', ALL)
    StrCopy(sp_octoprint_offline, 'OctoPrint is offline', ALL)
    StrCopy(sp_preheat, 'Nozzle preheating', ALL)
    StrCopy(sp_heat_off, 'Heating stopped', ALL)
    StrCopy(sp_pause, 'Print paused', ALL)
    StrCopy(sp_resume, 'Print resumed', ALL)
    StrCopy(sp_cancel, 'Print cancelled', ALL)
    StrCopy(sp_nozzle_heat, 'Nozzle heating requested', ALL)
    StrCopy(sp_insert, 'Filament insertion', ALL)
    StrCopy(sp_retract, 'Filament retraction', ALL)
    StrCopy(sp_emergency, 'Emergency stop', ALL)
    StrCopy(sp_connect, 'Printer connection', ALL)
ENDPROC
...
PROC upload_progress(force)
    ...
    IF force OR (percent <> g_upload_last_percent)
        ...
        StringF(g_result, 'Upload: \d %  (\d / \d KB)', percent, sent_k, total_k)
...
PROC has_http_success()
    ...
    /* Find HTTP/1.x 2xx near the response start, even if the stack adds noise. */
...
PROC request_reply(method, path, body, label)
    IF read_gui_config() = FALSE
        show_message('Enter an OctoPrint IP address and API key.')
        RETURN FALSE
    ENDIF
    ...
    IF has_http_success()
        StringF(g_result, '\s: command accepted.', label)
        show_message(g_result)
        RETURN TRUE
    ELSE
        StringF(g_result, '\s: OctoPrint refused the command.', label)
        show_message(g_result)
        RETURN FALSE
    ENDIF
ENDPROC FALSE
...
PROC test_octoprint()
    IF request_reply('GET', '/api/version', '', 'Test OctoPrint')
        list_clear(li_result)
        list_add(li_result, 'OctoPrint is online.')
    ENDIF
ENDPROC
...
PROC send_m105()
    IF request_reply('POST', '/api/printer/command', '{"commands":["M105"]}', 'M105')
        list_clear(li_result)
        list_add(li_result, 'M105 sent to the printer.')
        list_add(li_result, 'Command accepted by OctoPrint.')
        list_add(li_result, 'Click Status to read temperatures.')
    ENDIF
ENDPROC
...
PROC fetch_printer_status()
    ...
    StringF(g_status_state_line, 'State: \s', valeur_etat)
    StringF(g_status_tool_line, 'Nozzle: \s C / target \s C', valeur_buse, valeur_buse_cible)
    StringF(g_status_bed_line, 'Bed: \s C / target \s C', valeur_plateau, valeur_plateau_cible)
ENDPROC TRUE
...
PROC monitor_offline()
    ...
    ENDIF
    g_voice_temp_hot := FALSE
    StrCopy(g_status_state_line, 'State: OctoPrint offline', ALL)
    StrCopy(g_status_tool_line, 'Nozzle: -- C / target -- C', ALL)
    StrCopy(g_status_bed_line, 'Bed: -- C / target -- C', ALL)
    StrCopy(g_monitor_file_line, 'File: --', ALL)
    StrCopy(g_job_progress_line, 'Progress: -- %', ALL)
    StrCopy(g_job_left_line, 'Auto refresh stopped', ALL)
...
PROC read_printer_status()
    IF fetch_printer_status() = FALSE
        show_message('Status: unable to read OctoPrint.')
        monitor_offline()
        RETURN
    ENDIF
    ...
    voice_monitor_events()
    list_clear(li_result)
    list_add(li_result, 'PRINTER STATUS')
...
PROC fetch_job_status()
    ...
    IF (fichier[0] = 0) OR begins(fichier, 'null')
        g_job_file_raw[0] := 0
        IF etat[0]
            StringF(g_job_progress_line, 'Print: \s', etat)
        ELSE
            StrCopy(g_job_progress_line, 'Print: none', ALL)
        ENDIF
        StrCopy(g_job_left_line, 'Time left: --', ALL)
        StrCopy(g_monitor_file_line, 'File: --', ALL)
    ELSE
        short_percent(avance, g_progress_short)
        StringF(g_job_progress_line, 'Progress: \s %', g_progress_short)
        StringF(g_job_left_line, 'Time left: \s', restant_lisible)
        StringF(g_monitor_file_line, 'File: \s', fichier)
    ENDIF
ENDPROC TRUE
...
PROC read_job()
    ...
    IF request_reply('GET', '/api/job', '', 'Read job') = FALSE THEN RETURN
    ...
    list_clear(li_result)
    IF (fichier[0] = 0) OR begins(fichier, 'null')
        list_add(li_result, 'No print job running.')
        IF etat[0]
            StringF(g_job_state_line, 'Printer state: \s', etat)
    ...
    readable_time(restant, restant_lisible)
    list_add(li_result, 'PRINT JOB RUNNING')
    StringF(g_job_file_line, 'File: \s', fichier)
    list_add(li_result, g_job_file_line)
    StringF(g_job_state_line, 'State: \s', etat)
    list_add(li_result, g_job_state_line)
    short_percent(avance, g_progress_short)
    StringF(g_job_progress_line, 'Progress: \s %', g_progress_short)
    list_add(li_result, g_job_progress_line)
    StringF(g_job_time_line, 'Elapsed time: \s', temps_lisible)
...
PROC motors_off()
    request_reply('POST', '/api/printer/command', '{"commands":["M84"]}', 'Motors off')
ENDPROC

PROC pla_heat()
    IF request_reply('POST', '/api/printer/command', '{"commands":["M104 S180","M140 S70"]}', 'PLA preheat')
        voice_say(sp_preheat)
    ENDIF
ENDPROC

PROC heat_off()
    IF request_reply('POST', '/api/printer/command', '{"commands":["M104 S0","M140 S0"]}', 'Heating off')
...
PROC cancel_cleanup()
    Delay(100)
    IF request_reply('POST', '/api/printer/command', '{"commands":["M104 S0","M140 S0","G91","G1 Z10 F300","G90","G28 X0 Y0","M84"]}', 'Reset after cancel')
        list_clear(li_result)
        list_add(li_result, 'Print cancelled.')
        list_add(li_result, 'Heating off, Z lifted, X/Y axes parked.')
    ENDIF
ENDPROC

PROC cancel_print()
    IF request_reply('POST', '/api/job', '{"command":"cancel"}', 'Cancel')
        voice_say(sp_cancel)
        cancel_cleanup()
    ENDIF
ENDPROC
...
PROC preheat_nozzle()
    IF request_reply('POST', '/api/printer/command', '{"commands":["M104 S200"]}', 'Nozzle heat')
        voice_say(sp_nozzle_heat)
    ENDIF
ENDPROC

PROC insert_filament()
    IF request_reply('POST', '/api/printer/command', '{"commands":["M83","G1 E30 F120","M82"]}', 'Filament insert')
        voice_say(sp_insert)
    ENDIF
ENDPROC

PROC retract_filament()
    IF request_reply('POST', '/api/printer/command', '{"commands":["M83","G1 E-80 F600","M82"]}', 'Filament retract')
        voice_say(sp_retract)
    ENDIF
ENDPROC

PROC emergency_stop()
    IF request_reply('POST', '/api/printer/command', '{"commands":["M112"]}', 'EMERGENCY STOP M112')
        voice_say(sp_emergency)
        list_clear(li_result)
        list_add(li_result, 'EMERGENCY STOP sent.')
        list_add(li_result, 'Reset/reconnect the printer, then Connect printer.')
    ENDIF
ENDPROC

PROC connect_cleanup()
    Delay(100)
    IF request_reply('POST', '/api/printer/command', '{"commands":["M104 S0","M140 S0","G91","G1 Z10 F300","G90","G28 X0 Y0","M84"]}', 'Reset after connection')
        list_clear(li_result)
        list_add(li_result, 'Printer reconnected.')
        list_add(li_result, 'Heating off, Z lifted, X/Y axes parked.')
    ENDIF
ENDPROC

PROC raz_printer()
    IF request_reply('POST', '/api/printer/command', '{"commands":["M104 S0","M140 S0","G91","G1 Z10 F300","G90","G28 X0 Y0","M84"]}', 'Printer reset')
        list_clear(li_result)
        list_add(li_result, 'Printer reset done.')
        list_add(li_result, 'Heating off, Z lifted, X/Y axes parked.')
    ENDIF
ENDPROC

PROC connect_printer()
    IF request_reply('POST', '/api/connection', '{"command":"connect"}', 'Printer connection')
...
PROC show_help()
    list_clear(li_result)
    list_add(li_result, 'HELP - OctoControl Speak')
    list_add(li_result, 'Test: checks OctoPrint and the API key.')
    list_add(li_result, 'Connect printer: reconnects the printer after a restart.')
    list_add(li_result, 'Files then Print selected: starts a G-code already on OctoPrint.')
    list_add(li_result, 'The monitor can announce events with SAY.')
    list_add(li_result, 'EMERGENCY M112: immediate stop, use only in case of trouble.')
ENDPROC

PROC show_about()
    list_clear(li_result)
    list_add(li_result, 'OctoControl Speak EN 0.4')
    list_add(li_result, 'Controls OctoPrint from AmigaE and MUI.')
    list_add(li_result, 'Project by Denis Costils - 2026.')
    list_add(li_result, 'Contact: costils.denis@free.fr')
    list_add(li_result, 'Direct HTTP connection, no browser needed.')
ENDPROC
...
PROC fill_files_list()
            ...
            IF g_files[i] = 58
                ...
                IF g_files[i] = 34
                    ...
                    IF g_files[fin] = 34
                        ...
                        i := fin + 1
                    ELSE
                        /* Incomplete response: leave parser without looping forever. */
                        i := fin
                    ENDIF
                ELSE
                    i++
    ...
    IF nombre = 0 THEN list_add(li_files, '(no G-code file)')
    IF g_files[FILES_BUFFER-2]
        list_add(li_files, '(partial list: too many files)')
    ENDIF
ENDPROC
...
PROC read_files()
    ...
    list_clear(li_files)
    list_add(li_files, 'Reading files...')
    clear_memory(g_files, FILES_BUFFER)
    IF http_request('GET', '/api/files/local?recursive=false', '', g_files, FILES_BUFFER) = FALSE
        list_clear(li_files)
        list_add(li_files, '(OctoPrint does not respond)')
        show_message('Unable to read OctoPrint files.')
        RETURN
    ENDIF
    ...
    IF (has_http_success() = FALSE) AND (find_text(g_files, '"files"', 0) < 0)
        ...
        list_clear(li_files)
        list_add(li_files, '(OctoPrint error)')
        list_add(li_files, statut)
        StringF(g_result, 'OctoPrint refused file list: \s', statut)
        show_message(g_result)
        RETURN
    ENDIF
...
PROC print_named_file(nom, label)
    ...
    IF request_reply('POST', chemin, '{"command":"select","print":true}', label)
        list_clear(li_result)
        StringF(g_result, 'Print started: \s', nom)
        list_add(li_result, g_result)
    ENDIF
ENDPROC
...
PROC selected_octoprint_file()
    ...
    IF actif < 0
        show_message('Click Files then select a G-code file.')
        RETURN
    ENDIF

    doMethod(li_files, [MUIM_List_GetEntry, actif, {entree}])
    IF entree = NIL
        show_message('No OctoPrint file selected.')
        RETURN
    ENDIF
    IF begins(entree, '(')
        show_message('Select a real G-code file.')
        RETURN
    ENDIF
ENDPROC entree
...
PROC print_selected()
    ...
    print_named_file(entree, 'Selected print')
ENDPROC
...
PROC delete_selected_file()
    ...
    StringF(chemin, '/api/files/local/\s', nom_encode)
    show_message('Deleting file...')
    IF request_reply_quiet('DELETE', chemin, '')
        read_files()
        list_clear(li_result)
        StringF(g_result, 'File deleted: \s', entree)
        list_add(li_result, g_result)
    ELSE
        show_message('Delete refused or OctoPrint does not respond.')
    ENDIF
ENDPROC
...
PROC upload_gcode(chemin:PTR TO CHAR, nom:PTR TO CHAR)
    ...
    g_response[0] := 0
    StrCopy(g_upload_error, 'No response received during upload.', ALL)
    g_upload_size := 0
    g_upload_total := 0
    g_upload_sent := 0
    g_upload_errno := 0
    ...
    IF h = NIL
        StrCopy(g_upload_error, 'Unable to open G-code file.', ALL)
        RETURN FALSE
    ENDIF
    ...
    IF taille < 0
        Close(h)
        StrCopy(g_upload_error, 'Unable to read G-code file size.', ALL)
        RETURN FALSE
    ENDIF
    ...
    IF sock < 0
        Close(h)
        StrCopy(g_upload_error, 'Unable to open network socket.', ALL)
        RETURN FALSE
    ENDIF
    clear_memory(adresse, SIZEOF sockaddr_in)
    adresse.family := AF_INET
    ...
    IF Connect(sock, adresse, SIZEOF sockaddr_in) < 0
        CloseSocket(sock)
        Close(h)
        StrCopy(g_upload_error, 'OctoPrint does not respond to the upload connection.', ALL)
        RETURN FALSE
    ENDIF

    IF send_all(sock, requete) = FALSE
        ok := FALSE
        StrCopy(g_upload_error, 'Error sending upload HTTP header.', ALL)
    ENDIF
    IF ok AND (send_all(sock, partie) = FALSE)
        ok := FALSE
        StrCopy(g_upload_error, 'Error sending upload file start.', ALL)
    ENDIF
    ...
    WHILE (restant > 0) AND ok
        ...
        IF lu <= 0
            ok := FALSE
            StrCopy(g_upload_error, 'Read error during file transfer.', ALL)
        ELSE
            IF send_bytes(sock, g_upload_buffer, lu) = FALSE
                ok := FALSE
                StrCopy(g_upload_error, 'Connection closed during file transfer.', ALL)
            ENDIF
    ...
    IF ok AND (send_all(sock, fin) = FALSE)
        ok := FALSE
        StrCopy(g_upload_error, 'Error sending upload file end.', ALL)
    ENDIF
    IF ok
    ...
    IF ok OR begins(g_upload_error, 'Connection closed')
...
PROC upload_selected(start_print)
    ...
    IF source = NIL
        show_message('Enter the path of a G-code file.')
        RETURN
    ENDIF
    IF source[] = 0
        show_message('Enter the path of a G-code file.')
    ...
    IF nom[0] = 0
        show_message('Invalid G-code file name.')
        RETURN
    ENDIF
    IF is_gcode(nom) = FALSE
        show_message('Select a G-code file (.gcode, .gco or .gc).')
        RETURN
    ENDIF

    show_message('Uploading G-code file to OctoPrint...')
    IF upload_gcode(chemin, nom) = FALSE
        list_clear(li_result)
        list_add(li_result, 'G-code upload failed.')
        IF g_response[0]
            list_add(li_result, 'OctoPrint refused the upload.')
            http_status_line(g_result)
            list_add(li_result, g_result)
        ELSE
            list_add(li_result, g_upload_error)
            StringF(g_result, 'Selected path: \s', chemin)
            list_add(li_result, g_result)
        ENDIF
        RETURN
    ENDIF
    ...
    IF start_print
        print_named_file(nom, 'Upload and print')
    ELSE
        list_clear(li_result)
        StringF(g_result, 'File uploaded to OctoPrint: \s', nom)
        list_add(li_result, g_result)
        list_add(li_result, 'Click Files to refresh the list.')
    ENDIF
ENDPROC
...
PROC bouton_urgence()
    ...
    obj := TextObject,
        ButtonFrame,
        MUIA_Text_Contents, 'EMERGENCY M112',
        MUIA_Text_PreParse, '\ec',
        MUIA_InputMode, MUIV_InputMode_RelVerify,
...
PROC main() HANDLE
    ...
    ap_native := ApplicationObject,
        MUIA_Application_Title, 'OctoControl Speak EN',
        MUIA_Application_Version, '0.4',
        MUIA_Application_Author, 'Denis Costils',
        MUIA_Application_Description, 'Speaking direct OctoPrint control for AmigaE',
        MUIA_Application_Base, 'OCTOSPKEN',

        SubWindow,
            wi_native := WindowObject,
                MUIA_Window_Title, 'OctoControl Speak EN',
                MUIA_Window_ID, "OCSE",
                MUIA_Width, 560,
                MUIA_Height, 680,
                MUIA_Window_Menustrip, MenustripObject,
                    Child, MenuObject,
                        MUIA_Menu_Title, 'Help',
                        Child, mi_help := MenuitemObject,
                            MUIA_Menuitem_Title, 'Usage',
                        End,
                ...
                WindowContents, VGroup,
                    ...
                    Child, GroupObject,
                        ...
                        MUIA_FrameTitle, 'OctoPrint connection',
                        ...
                        End,
                        Child, TextObject, MUIA_Text_Contents, 'API key', End,
                    ...
                    Child, GroupObject,
                        MUIA_Frame, MUIV_Frame_Group,
                        MUIA_FrameTitle, 'Printer',
                        MUIA_Group_Columns, 2,
                        Child, bt_connect := bouton('Connect printer'),
                        Child, bt_raz := bouton('RAZ'),
                    End,

                    Child, GroupObject,
                        MUIA_Frame, MUIV_Frame_Group,
                        MUIA_FrameTitle, 'Read',
                    ...
                    Child, GroupObject,
                        MUIA_Frame, MUIV_Frame_Group,
                        MUIA_FrameTitle, 'OctoPrint files',
                        MUIA_Group_Columns, 3,
                        Child, bt_files := bouton('Files'),
                        Child, bt_printselected := bouton('Print selected'),
                        Child, bt_deletefile := bouton('Delete'),
                    End,
                    ...
                    Child, GroupObject,
                        MUIA_Frame, MUIV_Frame_Group,
                        MUIA_FrameTitle, 'Upload a G-code file',
                        Child, GroupObject,
                            MUIA_Group_Columns, 3,
                            Child, PopaslObject,
                            ...
                            End,
                            Child, bt_upload := bouton('Upload'),
                            Child, bt_uploadprint := bouton('Upload+Print'),
                        End,
                        ...
                        Child, tx_upload_progress := TextObject,
                            MUIA_Text_Contents, 'Upload: -- %',
                        End,
                    End,

                    Child, GroupObject,
                        MUIA_Frame, MUIV_Frame_Group,
                        MUIA_FrameTitle, 'Machine control',
                        MUIA_Group_Columns, 4,
                        Child, bt_home := bouton('Home'),
                        Child, bt_motorsoff := bouton('Motors OFF'),
                    ...
                    Child, HGroup,
                        Child, GroupObject,
                            MUIA_Frame, MUIV_Frame_Group,
                            MUIA_FrameTitle, 'Filament control',
                            MUIA_Group_Columns, 3,
                            Child, bt_prechauffe := bouton('Heat Nozzle'),
                            Child, bt_insertion := bouton('Insert Fil.'),
                            Child, bt_retrait := bouton('Retract Fil.'),
                        End,
        ...
        SubWindow,
            wi_monitor := WindowObject,
                MUIA_Window_Title, 'Printer monitor',
                MUIA_Window_ID, "OCMO",
                MUIA_Width, 360,
                MUIA_Height, 160,
                WindowContents, VGroup,
                    Child, GroupObject,
                        ...
                        Child, tx_monitor_state := TextObject,
                            MUIA_Text_Contents, 'State: --',
                        End,
                        Child, tx_monitor_tool := TextObject,
                            MUIA_Text_Contents, 'Nozzle: -- C / target -- C',
                        End,
                        Child, tx_monitor_bed := TextObject,
                            MUIA_Text_Contents, 'Bed: -- C / target -- C',
                        End,
                        Child, tx_monitor_file := TextObject,
                            MUIA_Text_Contents, 'File: --',
                        End,
                        Child, tx_monitor_job := TextObject,
                            MUIA_Text_Contents, 'Progress: -- %',
                        End,
                        Child, tx_monitor_left := TextObject,
                            MUIA_Text_Contents, 'Time left: --',
                        End,
                        Child, bt_monitor_main := bouton('Main'),
                    End,
                End,
            End,
        End
    ...
    show_message('Ready. Configuration is in S:octocontrol_native.cfg')
        ...
        IF result <> MUIV_Application_ReturnID_Quit
            SELECT result
                CASE ID_SAVE
                    IF read_gui_config()
                        IF save_config()
                            show_message('Configuration saved to S:octocontrol_native.cfg')
                        ELSE
                            show_message('Unable to write S:octocontrol_native.cfg')
                        ENDIF
                    ELSE
                        show_message('Enter an OctoPrint IP address and API key.')
                    ENDIF
...
EXCEPT
    ...
    SELECT exception
        CASE 1 ; WriteF('Unable to open muimaster.library.\n')
        CASE 2 ; WriteF('Unable to open bsdsocket.library.\n')
        CASE 3 ; WriteF('Unable to create the MUI application.\n')
    ENDSELECT
ENDPROC
